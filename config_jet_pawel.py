...
x_p = np.linspace(0.0, Lx, np.sqrt(num_particles),endpoint=False)
y_p = np.linspace(0.0, Ly, np.sqrt(num_particles),endpoint=False)
...
H_data= 0 * X + H0 # initialize to constant for now
...
Hmax_var[0] = np.amax(H_data)

assert( np.amax(H_data) > 0.0) # only makes sense if positive
